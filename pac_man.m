...
%                    event markers generated by Mobilab-based
%                    single-channel event marking tool developed for
%                    Michigan Children's Hospital (not generally available;
%                    if interested, please email Makoto Miyakoshi); 4, use
%                    user-selected event markers generated by EEGLAB. Use 1
...
analyAmp_noPeriBoundary   = analyAmp;
analyPhase_noPeriBoundary = analyPhase;
...
analyAmp_noPeriBoundary(:,periBoundaryIdx)   = [];
analyPhase_noPeriBoundary(:,periBoundaryIdx) = [];
...
tmpZ = analyAmp_noPeriBoundary.*exp(1i*analyPhase_noPeriBoundary);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    case 1
        analyAmpSort   = sort(analyAmp_noPeriBoundary, 2, 'descend');
        critical       = floor(length(analyAmpSort)*hfoTopRatio/100);
        criticalValues = analyAmpSort(:, critical);
        logicalHasMask = analyAmp_noPeriBoundary >= repmat(criticalValues, [1 size(analyAmp_noPeriBoundary,2)]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    case 4 
        ...
        winddowLengthInFrame = round(windowLength/(1000/EEG.srate));
        selectedWindowEdges  = [latencyIdx-winddowLengthInFrame latencyIdx+winddowLengthInFrame];

        % Exclude out of data range windows.
        outOfDataRangeWinIdx = find(selectedWindowEdges(:,1)<0 | selectedWindowEdges(:,2)>EEG.pnts);
        ...
        analyAmpSort   = sort(analyAmp_noPeriBoundary(:,inclusiveMask), 2, 'descend');
        critical       = floor(length(analyAmpSort)*hfoTopRatio/100);
        criticalValues = analyAmpSort(:, critical);
        logicalHasMask = analyAmp_noPeriBoundary >= repmat(criticalValues, [1 size(analyAmp_noPeriBoundary,2)]);
...
clear logical* tmp* critical* global* analy* x_* whichMarker windowLength
...
for chIdx = 1:EEG.nbchan
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % find their Angle
        analyAmp_noPeriBoundary   = abs(  EEG.pac.analyticEEG(chIdx,:));
        analyPhase_noPeriBoundary = angle(EEG.pac.analyticEEG(chIdx,:));
        hfoAmp     = analyAmp_noPeriBoundary(hfoIndex);
        hfoPhase   = analyPhase_noPeriBoundary(hfoIndex);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for surroIdx = 1:numSurro
            tmpSurroPhase   = [analyPhase_noPeriBoundary(skipIdx(surroIdx):end) analyPhase_noPeriBoundary(1:skipIdx(surroIdx)-1)]; % randomize phase instead of amp- consider circular shifts
        ...
        EEG.pac.uncorrected.MIpval(chIdx,1) = stat_surrogate_pvals(SurroMI, tmpMI, 'right');
